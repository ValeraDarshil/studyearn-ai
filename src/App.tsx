...
// Full_Achivements_SFX  → plays when toast opens (achievement unlocked)
// Achievements_Ending_SFX → plays when toast closes (button click OR auto-close)
...
function playAchievementSFX(_rarity?: string) {
  // Plays the open SFX — rarity param kept for API compatibility, not used
  try {
    if (!achOpenAudio) {
...
function AchievementToast({ achievement, onClose }: { achievement: any; onClose: () => void }) {
  ...
  const handleClose = useCallback(() => {
    playAchievementCloseSFX();   // Achievements_Ending_SFX on close
    setVisible(false);
  ...
  useEffect(() => {
    ...
    playAchievementSFX();
    // Auto-close after 6s — ending SFX fires via handleClose
