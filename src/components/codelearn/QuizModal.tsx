    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#0f0f1a] border border-white/10 rounded-2xl max-w-xl w-full overflow-hidden">
        ...
        <div className="p-6">
          ...
          {/* Explanation — wrong reason + correct reason */}
          {showExplanation && (
            <>
              {selectedAnswer !== null && selectedAnswer !== question.correct && question.wrong_explanations?.[selectedAnswer] && (
                <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-4 mb-3 animate-fade-in">
                  <div className="text-xs text-red-400 font-medium mb-1.5">❌ Yeh isliye galat tha:</div>
                  <p className="text-gray-400 text-sm leading-relaxed">{question.wrong_explanations[selectedAnswer]}</p>
                </div>
              )}
              {question.explanation && (
                <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4 mb-4 animate-fade-in">
                  <div className="text-xs text-blue-400 font-medium mb-1.5">✅ Sahi jawab kyun:</div>
                  <p className="text-gray-400 text-sm leading-relaxed">{lang === 'en' && question.explanation_en ? question.explanation_en : question.explanation}</p>
                </div>
              )}
            </>
