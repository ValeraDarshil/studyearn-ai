...
export default function QuizModal({
  ...
  const [quizPhase, setQuizPhase] = useState<'question' | 'result'>('question');
  ...
  if (quizPhase === 'result') {
    return (
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-4">
        <div className="bg-[#0f0f1a] border border-white/10 rounded-t-2xl sm:rounded-2xl p-5 sm:p-8 max-w-md w-full text-center">
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-4">
      <div className="bg-[#0f0f1a] border border-white/10 rounded-t-2xl sm:rounded-2xl max-w-xl w-full overflow-hidden">
        {/* Header */}
        ...
        {/* Question */}
        <div className="p-6">
          <h3 className="text-white font-semibold text-base md:text-lg leading-relaxed mb-4 md:mb-6">
            ...
            {(lang === 'en' && question.options_en ? question.options_en : question.options).map((option, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 md:gap-3 px-3 md:px-4 py-3 md:py-3.5 rounded-xl border text-left transition-all duration-200 text-sm ${style} ${selectedAnswer === null ? 'cursor-pointer' : 'cursor-default'}`}
                >
