...
import { Brain, Presentation, FileText, Gift, Trophy, User, LogOut, Sparkles, Zap, Menu, X, Flame as FlameIcon, BookOpen, HelpCircle, BarChart2, FlaskConical, NotebookPen, Users, Code2 } from 'lucide-react';
...
export function DashboardLayout() {
  ...
  const { points, questionsLeft, streak, isPremium } = useApp();
  ...
  const navItemsFull = [
    ...
    { icon: Code2,        label: 'Code Learn',      path: '/codelearn', badge: 'NEW', isExternal: true },
    ...
    <div className="min-h-screen animated-bg grid-bg">
      ...
      >
        ...
        <div className="mx-4 glass rounded-xl p-4 border border-white/5 mb-4">
          ...
          {isPremium && (
            <div className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-yellow-300 px-2 py-1 rounded-lg"
        ...
        <div className="sticky top-0 z-30 glass border-b border-white/5 px-3 md:px-8 py-3 md:py-4 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <NavLink to="/app/profile">
              ...
              >
                {profileHovered && (
                  <div className="profile-spin-ring absolute inset-0 rounded-full"
                    style={{ background: 'conic-gradient(from 0deg, #8b5cf6, #3b82f6, #06b6d4, #8b5cf6)', padding: '2px', borderRadius: '9999px', zIndex: 0 }}>
                    <div style={{ width: '100%', height: '100%', borderRadius: '9999px', background: '#030712' }} />
                  </div>
                )}
                {profileHovered && (
                  <div className="profile-glow-pulse absolute inset-0 rounded-full"
                    style={{ background: 'radial-gradient(circle, rgba(139,92,246,0.35) 0%, transparent 70%)', filter: 'blur(6px)', zIndex: 0 }} />
                )}
                ...
                {profileHovered && (
                  <div className="profile-tooltip absolute pointer-events-none"
                    style={{ bottom: '-32px', left: '50%', whiteSpace: 'nowrap', background: 'rgba(15,10,30,0.92)', border: '1px solid rgba(139,92,246,0.35)', borderRadius: '8px', padding: '3px 10px', fontSize: '11px', fontWeight: 500, color: '#c4b5fd', backdropFilter: 'blur(8px)', zIndex: 50 }}>
                    My Profile
                  </div>
                )}
              </div>
