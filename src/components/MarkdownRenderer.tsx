...
// ─── Strip leftover label prefixes the AI sometimes outputs ──
// e.g. "📖 EXPLANATIONThat's a great point" → "That's a great point"
// e.g. "1. 📖 EXPLANATION\nThe concept..." → "\nThe concept..."
function stripLabelPrefixes(md: string): string {
  return md
    // "📖 EXPLANATIONSome text" — label glued to text, no space
    .replace(/[📖💡⚡❓🔥]\s*(EXPLANATION|EXPLAIN|EXAMPLE|EXAMPLES?|QUICK\s*SUMMARY|SUMMARY|QUICK\s*CHECK|MICRO-LEARNING\s*HOOK)\s*/gi, "")
    // "1. 📖 EXPLANATION" or "1. EXPLANATION" as standalone line prefix
    .replace(/^\d+\.\s*[📖💡⚡❓🔥]?\s*(EXPLANATION|EXPLAIN|EXAMPLE|EXAMPLES?|QUICK\s*SUMMARY|SUMMARY|QUICK\s*CHECK)\s*$/gim, "")
    // Plain "EXPLANATION" at very start of a line glued to text
    .replace(/^(EXPLANATION|EXAMPLE|QUICK SUMMARY|QUICK CHECK)\s*/gim, "")
    // numbered section like "1. EXPLANATION\n" or "2. EXAMPLE\n"
    .replace(/^\d+\.\s+(EXPLANATION|EXAMPLE|QUICK SUMMARY|QUICK CHECK)\n/gim, "\n");
}

// ─── Fallback parser (Mode B) ─────────────────────────────────
function parseFallback(md: string): string {
  const cleaned = stripLabelPrefixes(md);
  ...
  let pq=""; let mqOpts: string[]=[];
  let buf: string[]=[];
  let firstParaDone = false; // tracks if we've rendered the first paragraph yet
  ...
  const flushBuf=()=>{
    ...
    if(!joined) return;
    if(!firstParaDone) {
      // First paragraph — render as premium lead regardless of style detection
      firstParaDone = true;
      const inner = renderInline(joined);
      out.push(`<p class="seg seg-lead seg-first-lead">${inner}</p>`);
    } else {
      out.push(renderFallbackPara(joined));
    }
    buf=[];
  };
  const flushMcq=()=>{
...
/* ── Premium first paragraph (opening hook) ── */
.seg-first-lead{
  font-size:.975rem;
  font-weight:500;
  color:#f1f5f9;
  ...
  margin-bottom:.65rem;
  background:linear-gradient(90deg,rgba(139,92,246,.07) 0%,transparent 70%);
  border-radius:0 8px 8px 0;
  animation:leadIn .35s ease-out;
}
@keyframes leadIn{from{opacity:0;transform:translateX(-4px)}to{opacity:1;transform:translateX(0)}}
