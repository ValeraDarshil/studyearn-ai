...
/**
 * StudyEarn AI — MarkdownRenderer v6
 * ─────────────────────────────────────────────────────────────
 * Fixes in v6:
 * ✅ FIX 1 — No more class="syn-str" leaking during streaming
 *    → Incomplete ``` blocks rendered as plain <pre> until closing ``` arrives
 * ✅ FIX 2 — Smooth streaming (no double-render glitch)
 *    → Removed dangerouslySetInnerHTML, only useEffect updates innerHTML
 * ✅ Syntax highlighting — Python, JS/TS, HTML, CSS, C/C++, Java, SQL, Bash
 * ✅ Copy button on code blocks
 * ✅ KaTeX math rendering ($inline$ and $$block$$)
 * ✅ Bold, italic, inline code, links, strikethrough
 * ✅ H1/H2/H3, blockquotes, bullet/numbered lists, tables, HR
 * ✅ Emoji-aware line highlights (⚠️ warning, 📌 key, ✅ success, ❌ error)
 */
...
// ─── Syntax highlighter ────────────────────────────────────────
// IMPORTANT: input must be HTML-escaped already.
// Regexes only wrap escaped text in <span> — they never produce
// raw HTML attributes, so partial streaming cannot leak class= text.
function syntaxHighlight(escaped: string, lang: string): string {
  const l = lang.toLowerCase();

  if (l === "python" || l === "py") {
    return escaped
      // strings first (before keywords touch them)
      .replace(/(&#34;&#34;&#34;[\s\S]*?&#34;&#34;&#34;|&#39;&#39;&#39;[\s\S]*?&#39;&#39;&#39;|&#34;[^&#34;\n]*&#34;|&#39;[^&#39;\n]*&#39;|&quot;[^&\n]*&quot;)/g,
        '<span class="syn-str">$1</span>')
      .replace(/("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g, '<span class="syn-str">$1</span>')
      .replace(/\b(def|class|import|from|return|if|elif|else|for|while|in|not|and|or|is|None|True|False|try|except|finally|with|as|pass|break|continue|raise|lambda|yield|async|await|global|nonlocal|del|assert)\b/g,
        '<span class="syn-kw">$1</span>')
      .replace(/\b(print|input|len|range|type|int|float|str|list|dict|set|tuple|bool|open|sum|max|min|abs|round|sorted|enumerate|zip|map|filter|super|self|cls)\b/g,
  ...
  if (["js","javascript","ts","typescript","jsx","tsx"].includes(l)) {
    return escaped
      ...
      .replace(/\b(const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|new|class|extends|import|export|default|from|async|await|try|catch|finally|throw|typeof|instanceof|in|of|this|super|static|null|undefined|true|false|void|delete|yield)\b/g,
        '<span class="syn-kw">$1</span>')
      .replace(/\b(console|document|window|Math|Array|Object|String|Number|Boolean|Promise|JSON|Date|Error|Map|Set|fetch|setTimeout|clearTimeout|setInterval|parseInt|parseFloat)\b/g,
        '<span class="syn-builtin">$1</span>')
      .replace(/\b(\d+\.?\d*)\b/g, '<span class="syn-num">$1</span>')
      .replace(/(\/\/[^\n]*)/g, '<span class="syn-comment">$1</span>')
  ...
  if (["c","cpp","c++"].includes(l)) {
    return escaped
      .replace(/(["'].*?["'])/g, '<span class="syn-str">$1</span>')
      .replace(/\b(int|float|double|char|void|bool|long|short|unsigned|signed|struct|class|return|if|else|for|while|do|switch|case|break|continue|include|define|typedef|enum|const|static|extern|auto|NULL|true|false|new|delete|namespace|using|public|private|protected|virtual)\b/g,
        '<span class="syn-kw">$1</span>')
      .replace(/(#\w+)/g, '<span class="syn-deco">$1</span>')
  ...
  if (l === "java") {
    return escaped
      .replace(/(["'].*?["'])/g, '<span class="syn-str">$1</span>')
      .replace(/\b(public|private|protected|class|interface|extends|implements|import|package|return|if|else|for|while|do|switch|case|break|continue|new|void|static|final|abstract|try|catch|finally|throw|throws|instanceof|this|super|null|true|false|int|float|double|char|boolean|long|short|byte|String)\b/g,
        '<span class="syn-kw">$1</span>')
      .replace(/\b(\d+\.?\d*[fFlL]?)\b/g, '<span class="syn-num">$1</span>')
      .replace(/(\/\/[^\n]*)/g, '<span class="syn-comment">$1</span>')
      .replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="syn-comment">$1</span>');
  ...
  if (l === "sql") {
    return escaped
      .replace(/('[^']*')/g, '<span class="syn-str">$1</span>')
      .replace(/\b(SELECT|FROM|WHERE|JOIN|LEFT|RIGHT|INNER|OUTER|ON|GROUP|ORDER|BY|HAVING|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|DROP|ALTER|INDEX|PRIMARY|KEY|FOREIGN|REFERENCES|NOT|NULL|AND|OR|IN|LIKE|BETWEEN|AS|DISTINCT|COUNT|SUM|AVG|MAX|MIN|LIMIT|OFFSET|UNION|ALL|EXISTS|CASE|WHEN|THEN|ELSE|END)\b/gi,
        '<span class="syn-kw">$1</span>')
      .replace(/\b(\d+\.?\d*)\b/g, '<span class="syn-num">$1</span>')
      .replace(/(--[^\n]*)/g, '<span class="syn-comment">$1</span>');
  }
  ...
  if (["bash","sh","shell","zsh"].includes(l)) {
    return escaped
      .replace(/(["'].*?["'])/g, '<span class="syn-str">$1</span>')
      .replace(/\b(if|then|else|elif|fi|for|while|do|done|case|esac|function|return|exit|echo|cd|ls|mkdir|rm|cp|mv|grep|sed|awk|cat|chmod|chown|export|source|alias)\b/g,
        '<span class="syn-kw">$1</span>')
      .replace(/(\$\{?[\w]+\}?)/g, '<span class="syn-builtin">$1</span>')
      .replace(/(#[^\n]*)/g, '<span class="syn-comment">$1</span>');
  }

  return escaped; // unknown language — return escaped plain text
}
...
function renderInline(text: string): string {
  return text
    // inline math — before any other processing
...
// ─── Main parser ───────────────────────────────────────────────
// KEY FIX: Incomplete code fences (streaming) are rendered as
// plain <pre> without syntax highlighting — so no <span> HTML
// can ever leak as raw text into the UI mid-stream.
function parse(md: string): string {
  ...
  while (i < lines.length) {
    ...
    if (t.startsWith("```")) {
      ...
      // Collect lines until closing ``` OR end of string (streaming — incomplete block)
      let closed = false;
      while (i < lines.length) {
      ...
      if (closed) {
        // ✅ Complete block — safe to syntax-highlight
        const highlighted = syntaxHighlight(escapeHtml(rawCode), lang);
        out.push(
          `<div class="md-cb" data-cb>` +
          `<div class="md-cbh"><span class="md-lang">${escapeHtml(lang)}</span>` +
          `<button class="md-cpb" data-cpb>Copy</button></div>` +
          `<pre class="md-pre"><code class="md-code">${highlighted}</code></pre>` +
          `</div>`
        );
      } else {
        // ⚡ Incomplete block (still streaming) — render as plain escaped text
        // This prevents any syn-str / syn-kw HTML from leaking as visible text
        out.push(
          `<div class="md-cb md-cb-streaming" data-cb>` +
          `<div class="md-cbh"><span class="md-lang">${escapeHtml(lang)}</span>` +
          `<span class="md-streaming-badge">…streaming</span></div>` +
          `<pre class="md-pre"><code class="md-code">${escapeHtml(rawCode)}</code></pre>` +
          `</div>`
        );
      }
      continue;
...
.md-cb-streaming{border-color:rgba(99,102,241,.25);animation:md-pulse 1.5s ease-in-out infinite}
@keyframes md-pulse{0%,100%{border-color:rgba(99,102,241,.15)}50%{border-color:rgba(99,102,241,.4)}}
...
.md-streaming-badge{font-size:.65rem;color:#6366f1;animation:md-pulse 1s ease-in-out infinite}
...
function injectCSS() {
  ...
  document.getElementById("sea-md-v5")?.remove();
  el.textContent = CSS;
...
export function MarkdownRenderer({ content, className = "" }: Props) {
  ...
  // FIX 2: Only update via useEffect — never use dangerouslySetInnerHTML simultaneously.
  // Using both causes double-render flicker/glitch during streaming.
  const doRender = useCallback(() => {
    if (!ref.current) return;
    const html = parse(content);
    // Only update DOM if content actually changed (avoids unnecessary repaints)
    if (ref.current.dataset.lastHash !== content.length.toString() + content.slice(-20)) {
      ref.current.innerHTML = html;
      ref.current.dataset.lastHash = content.length.toString() + content.slice(-20);
  ...
  useEffect(() => {
    doRender();
    // KaTeX: only load if math syntax present
    if (content.includes("$")) {
      loadKatex().then(() => {
        if (ref.current && (window as any).katex) doRender();
      });
    }
  }, [content, doRender]);

  // Empty div — innerHTML managed entirely by useEffect above
