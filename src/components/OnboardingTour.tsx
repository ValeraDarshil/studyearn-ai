 ...
 *
 * Persistence strategy (3 layers, most reliable first):
 *   1. DB field  user.onboardingCompleted = true  (server, cross-device)
 *   2. localStorage flag  se_tour_done            (instant, no flicker on refresh)
 *   3. React state showOnboarding in App.tsx       (runtime gate)
 *
 * On complete/skip → calls POST /api/user/complete-onboarding
 *                  → sets localStorage
 *                  → App.tsx removes tour from DOM
 *
 * NO useNavigate() — was breaking HashRouter.
 */
...
// ─── localStorage helpers (Layer 2 — instant, no flicker) ────────────────────
...
function SpotlightOverlay({
  ...
  return (
    <>
      <svg
        onClick={onBackdropClick}
        style={{ position: 'fixed', inset: 0, width: vw, height: vh, zIndex: 9990, overflow: 'hidden', cursor: 'default' }}
      >
        <defs>
          <mask id="tour-mask">
...
function TooltipCard({
  ...
  const CARD_W = 340, APPROX_H = 230, GAP = 16, EDGE = 12;
  ...
  return (
    <div style={{ ...getCardStyle(), animation: 'tourCardIn 0.42s cubic-bezier(0.34,1.56,0.64,1) both' }}>
      <div style={{
        ...
        border: `1.5px solid ${step.accentColor}40`, borderRadius: 20, padding: '20px 22px 18px',
        ...
        {/* Shine */}
        <div style={{ position: 'absolute', top: 0, bottom: 0, width: 70, pointerEvents: 'none', background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.035), transparent)', transform: 'skewX(-12deg)', animation: 'tourShine 3.5s ease-in-out 0.5s infinite' }} />

        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, fontSize: 18, background: `${step.accentColor}18`, border: `1px solid ${step.accentColor}30`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{step.emoji}</div>
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: step.accentColor }}>Step {stepIndex + 1} of {totalSteps}</span>
          </div>
          <button onClick={onSkip} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: 'rgba(148,163,184,0.55)', padding: '4px 8px', borderRadius: 6, transition: 'color 0.2s' }}
        ...
        <h3 style={{ margin: '0 0 8px', fontSize: 17, fontWeight: 700, color: '#f1f5f9', lineHeight: 1.3, fontFamily: "'Syne', system-ui, sans-serif" }}>{step.title}</h3>
        <p style={{ margin: '0 0 18px', fontSize: 13.5, color: 'rgba(148,163,184,0.88)', lineHeight: 1.65 }}>{step.description}</p>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div key={i} style={{ height: 6, borderRadius: 9999, width: i === stepIndex ? 22 : 6, background: i === stepIndex ? step.accentColor : 'rgba(148,163,184,0.18)', transition: 'all 0.35s cubic-bezier(0.34,1.56,0.64,1)' }} />
            ))}
          </div>
          <button
            onClick={onNext}
            style={{ background: `linear-gradient(135deg, ${step.accentColor}, ${step.accentColor}bb)`, color: '#fff', border: 'none', borderRadius: 12, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', letterSpacing: '0.01em', boxShadow: `0 4px 18px ${step.accentColor}44`, transition: 'all 0.2s ease' }}
...
export function OnboardingTour({ onComplete }: { onComplete: () => void }) {
  ...
  // Called on both "Start Learning" and "Skip tour"
  const finish = useCallback(() => {
    // Layer 2: save instantly to localStorage (prevents flicker on next refresh)
    saveLocalCompletion(userId);
    // Layer 1: save to DB in background (cross-device, permanent)
    completeOnboarding().catch(() => { /* silently ignore network errors */ });
    setMounted(false);
    // Small delay for exit feel, then remove tour from DOM
  ...
  return (
    <>
      ...
      <TooltipCard
        ...
        onNext={() => { if (isLast) finish(); else setStepIndex(i => i + 1); }}
