/** 
 *HTML Course — Week 10: JavaScript APIs — Fetch, Promises, Async/Await
 */

export const HTML_WEEK_10 = {
  week: 10,
  title: 'JavaScript APIs — Internet se Data Lao',
  title_en: 'JavaScript APIs — Fetching Data from the Internet',
  description: 'Fetch API se real-world data lao aur dynamic webpages banao!',
  description_en: 'Use the Fetch API to get real-world data and build dynamic webpages!',
  xpReward: 260,
  sections: [
    {
      id: 'html-w10-s1',
      title: 'Fetch API aur Promises',
      title_en: 'Fetch API and Promises',
      emoji: '🌐',
      content: `## Fetch API — Internet se Data Lao!
...
async function loadUsers() {
  ...
  try {
    ...
    container.innerHTML = \`<div class="error">❌ Data load nahi hua: \${error.message}</div>\`;
  }
}
\`\`\`

### POST Request (Data Bhejno)
...
async function createPost(title, body) {
  try {
    ...
    const data = await res.json();
    console.log('Created:', data);  // { id: 101, title: ..., body: ... }
    return data;
  } catch (error) {
    console.error('Error:', error);
  }
...
### Promise.all — Multiple APIs Ek Saath
...
\`\`\``,
      content_en: `## Fetch API — Get Data from the Internet!
...
### Promise.all — Multiple APIs at Once
  ...
  <script>
    ...
    loadUsers();
      ...
      task: {
        ...
        hint: 'restcountries.com/v3.1/all se sab countries milte hain. flag: country.flags.png. population: Intl.NumberFormat().format(num) se formatted number milta hai. Modal: position:fixed; inset:0; background:rgba(0,0,0,0.7); z-index:100.',
        hint_en: 'restcountries.com/v3.1/all returns all countries. Flag: country.flags.png. Population: Intl.NumberFormat().format(num) gives a formatted number. Modal: position:fixed; inset:0; background:rgba(0,0,0,0.7); z-index:100.',
      },
      quiz: [
        {
          q: 'async/await aur .then() mein kya fark hai?',
          options: ['Koi fark nahi', 'Dono Promises handle karte hain — async/await zyada readable aur synchronous-looking code deta hai', 'async/await faster hai', '.then() deprecated hai'],
          correct: 1,
          explanation: 'Dono ka same result hai. async/await code ko synchronous-looking banata hai — nested .then() chains se zyada readable. Error handling: try/catch vs .catch(). Modern code mein async/await prefer karte hain.',
        ...
        {
          q: 'response.json() kya karta hai?',
          options: ['JSON file save karta hai', 'Response body ko JavaScript object/array mein convert karta hai', 'JSON validate karta hai', 'Response header read karta hai'],
          correct: 1,
          explanation: 'fetch() Response object return karta hai. .json() response body ko string se parse karke JavaScript object/array mein convert karta hai. Yeh bhi async hai, isliye await karte hain.',
        ...
        {
          q: 'Promise.all() kab use karte hain?',
          options: ['Ek API call ke liye', 'Multiple async operations parallel mein run karne ke liye — sab complete hone ka intezaar', 'Error handle karne ke liye', 'API cancel karne ke liye'],
          correct: 1,
          explanation: 'Promise.all([p1, p2, p3]) — sab Promises parallel mein run karta hai aur sab ke complete hone ka wait karta hai. Sequential await se tez hota hai. Agar koi bhi fail ho toh poora fail.',
...

    {
      ...
      emoji: '✨',
      content: `## Modern JavaScript — ES6+ ki Best Features!
...
// city default value hai agar user.city undefined ho

// Function parameter destructuring
function greet({ name, age }) {
  return \`Hello \${name}, tum \${age} saal ke ho!\`;
}
...
const users = [
  ...
  { name: 'Priya', age: 19, score: 92 },
  { name: 'Arjun', age: 23, score: 78 },
];
...
// findIndex
const idx = users.findIndex(u => u.name === 'Arjun');

// some — koi ek match karta hai?
const hasTopScorer = users.some(u => u.score >= 90);
...
\`\`\`js
// utils.js — export karo
export const formatDate = (date) => new Date(date).toLocaleDateString();
export const formatNumber = (n) => n.toLocaleString('en-IN');
export default class APIService { ... }

// main.js — import karo
import APIService, { formatDate, formatNumber } from './utils.js';
\`\`\`

### Optional Chaining aur Nullish Coalescing

\`\`\`js
const user = null;

// Bina optional chaining — error!
...
// Combine karo
const display = user?.profile?.avatar ?? '/default-avatar.png';
\`\`\``,
      content_en: `## Modern JavaScript — The Best ES6+ Features!
...
\`\`\`js
const rahul  = users.find(u => u.name === 'Rahul');
const hasTop = users.some(u => u.score >= 90);
const allAd  = users.every(u => u.age >= 18);
...
// main.js
import APIService, { formatDate } from './utils.js';
\`\`\`
...
const display = user?.profile?.avatar ?? '/default.png';
\`\`\``,
      codeExample: `<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <script>
    ...
    \`;
      ...
      task: {
        description: 'Ek "StudyEarn Dashboard" page banao ES6+ features use karke. JSONPlaceholder API se data fetch karo. Dashboard show kare: (1) Users list — destructuring se name/email/company nikalo, (2) Posts summary — find karke user ke posts count dikhao, (3) Todo stats — filter karke completed vs pending count, (4) Leaderboard — users ko sort karo algo-generated score se (todos completed / total * 100), (5) Skills cloud — flatMap se all unique company domains nikalo, (6) Search bar — real-time filter users by name.',
        ...
        hint: 'Promise.all se users, todos, posts ek saath fetch karo. User posts count: posts.filter(p => p.userId === user.id).length. Spread: [...new Set(arr)] se unique values nikalo. Optional chaining: user?.company?.name ?? "Unknown".',
        hint_en: 'Fetch users, todos, and posts at once with Promise.all. User post count: posts.filter(p => p.userId === user.id).length. Spread: [...new Set(arr)] for unique values. Optional chaining: user?.company?.name ?? "Unknown".',
      },
      quiz: [
        {
          q: 'Spread operator (...) array sort karne se pehle kyun use karte hain?',
          options: ['Performance ke liye', 'Sort original array mutate karta hai — spread se copy pe sort karo', 'Syntax ke liye', 'Sort kaam nahi karta bina spread ke'],
          correct: 1,
          explanation: 'Array.sort() original array ko in-place sort karta hai (mutate). [...arr].sort() pehle copy banata hai phir sort karta hai — original array unchanged rehta hai. React mein bhi yahi best practice hai!',
        ...
        {
          q: '[...new Set(array)] kya karta hai?',
          options: ['Array shuffle karta hai', 'Duplicate values hata ke unique values ka new array deta hai', 'Array reverse karta hai', 'Array khali karta hai'],
          correct: 1,
          explanation: 'Set sirf unique values store karta hai. new Set([1,2,2,3]) → Set{1,2,3}. Spread se wapis array mein convert karo: [...new Set([1,2,2,3])] → [1,2,3].',
        ...
        {
          q: 'Object destructuring mein default value kaise dete hain?',
          options: ['{ name = "default" } = obj', '{ name: "default" } = obj', '{ name || "default" } = obj', 'name = obj.name || "default"'],
          correct: 0,
          explanation: '{ name = "default" } = obj — agar obj.name undefined hai toh name ki value "default" hogi. Colon (:) rename ke liye hota hai: { name: userName } = obj — obj.name ko userName variable mein store karo.',
    ...
    {
      id: 'html-w10-s3',
      title: 'JavaScript Error Handling aur Debugging',
      title_en: 'JavaScript Error Handling and Debugging',
      emoji: '🐛',
      content: `## Error Handling — Robust Code Likhna!
...
async function fetchData(url) {
  try {
    ...
    if (error.name === 'TypeError') {
      console.error('Network error — internet connection check karo');
    } else if (error.message.includes('404')) {
      console.error('Data nahi mila — URL check karo');
    } else {
    ...
    // Hamesha run hoga — loading state hatao
    hideLoader();
  }
}
\`\`\`
...
console.table(users);           // Table format mein dikhao
...
function buggyFunction() {
  debugger;  // Browser DevTools mein automatically breakpoint
  return calculation;
}
...
typeof 'hi';       // 'string'
typeof [];         // 'object'
Array.isArray([]); // true (better check)
\`\`\`

### Common JS Bugs aur Fixes
...
// Bug 2: Async without await
const data = fetch(url);    // ❌ Promise object, not data!
const data = await fetch(url); // ✅

// Bug 3: Array mutation in state
state.items.push(item);     // ❌ mutates original
state.items = [...state.items, item]; // ✅ new array

// Bug 4: this in callbacks
const obj = {
  name: 'Rahul',
  greet: function() {
    setTimeout(function() {
    ...
    setTimeout(() => {
      console.log(this.name); // ✅ arrow function uses parent this
    }, 1000);
  }
};

// Bug 5: typeof null
typeof null === 'object' // true — gotcha!
null === null            // true — correct check
\`\`\``,
      content_en: `## Error Handling — Writing Robust Code!
...
async function fetchData(url) {
  try {
    ...
    if (error.name === 'TypeError')        console.error('Network error — check connection');
    else if (error.message.includes('404')) console.error('Not found — check URL');
...
debugger;  // automatic breakpoint in DevTools

Array.isArray([]);  // true (better than typeof)
\`\`\`

### Common JS Bugs and Fixes
...
<body>
  ...
  <script>
    ...
    showTable();
      ...
      task: {
        description: 'Apni Country Explorer app mein robust error handling add karo: (1) Network offline hone pe friendly error message, (2) API slow hone pe retry button (3 attempts), (3) Custom APIError class banao, (4) Loading states: shimmer skeleton, (5) Empty states: search result na mile toh friendly message, (6) Error boundary component — ek try/catch wrapper jo kisi bhi error ko gracefully handle kare, (7) console.table() se data DevTools mein dekho, (8) performance timing — console.time() se fetch kitna time le raha hai measure karo.',
        ...
        hint_en: 'Network check: navigator.onLine. Retry: for(let i=0;i<3;i++) { try { return await fetch(); } catch(e) { if(i===2) throw e; await sleep(1000); } }. sleep: const sleep = ms => new Promise(r => setTimeout(r, ms)).',
      },
      quiz: [
        {
          q: 'finally block kab run hota hai?',
          options: ['Sirf success pe', 'Sirf error pe', 'Hamesha — success ya error dono cases mein', 'Kabhi nahi'],
          correct: 2,
          explanation: 'finally block hamesha run hota hai — try successful ho ya catch mein error aaye. Loading spinner hide karne, resources cleanup karne, logs lene ke liye perfect!',
        ...
        {
          q: '=== (strict equality) aur == (loose equality) mein kya fark hai?',
          options: ['Koi fark nahi', '=== type aur value dono check karta hai, == sirf value check karta hai (type coercion karta hai)', '== faster hai', '=== sirf numbers ke liye hai'],
          correct: 1,
          explanation: '0 == false → true (loose), 0 === false → false (strict). "1" == 1 → true (loose), "1" === 1 → false (strict). Hamesha === use karo unexpected results se bachne ke liye!',
        ...
        {
          q: 'navigator.onLine kya detect karta hai?',
          options: ['User logged in hai ya nahi', 'Browser internet se connected hai ya nahi', 'Page online hai ya nahi', 'Server status'],
          correct: 1,
          explanation: 'navigator.onLine = true agar browser internet se connected hai. False agar offline. window.addEventListener("offline", handler) se offline event bhi sun sakte ho.',
    ...
    {
      id: 'html-w10-s4',
      title: 'Week 10 Project — Weather App',
      title_en: 'Week 10 Project — Weather App',
      emoji: '🌤️',
      content: `## Week 10 Project — Weather App with Real API!

Ab tak JS mein seekha:
- Fetch API aur async/await
- ES6+ features
- Error handling aur debugging

In sab ko combine karke ek **real Weather App** banao!

### Free Weather API — OpenWeatherMap

\`\`\`
Sign up free: openweathermap.org/api
...
{
  "name": "Mumbai",
  "main": { "temp": 32, "humidity": 78, "feels_like": 36 },
  "weather": [{ "main": "Haze", "description": "mist", "icon": "50d" }],
  "wind": { "speed": 12 },
  "sys": { "country": "IN", "sunrise": 1234, "sunset": 5678 }
}
\`\`\`
...
1. Search Bar
   - City name input
   - Enter key ya Search button
   - Recent searches (localStorage)

2. Current Weather Card
   - Weather icon (openweathermap.org/img/wn/{icon}@2x.png)
   - Temperature (Celsius/Fahrenheit toggle)
   - Humidity, Wind speed, Feels like

3. 5-Day Forecast (Another endpoint)
...
5. Extras (Bonus)
   - Geolocation — current location ka weather
   - Background change based on weather
   - Day/Night mode (sunrise/sunset API data se)
\`\`\`
...
\`\`\``,
      content_en: `## Week 10 Project — Weather App with Real API!
...
Combine all of these to build a **real Weather App**!
...
\`\`\`
1. Search Bar — city name, Enter key, recent searches
2. Current Weather Card — icon, temp, humidity, wind
3. 5-Day Forecast (separate endpoint)
4. Error States — city not found, network error, loading
5. Bonus — geolocation, background by weather, day/night mode
\`\`\``,
      codeExample: `<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <script>
    ...
    search('mumbai');
      ...
      task: {
        ...
        hint: 'API_KEY environment variable ya direct string mein rakh sakte ho (sirf learning ke liye). Temp toggle: (celsius * 9/5 + 32).toFixed(1). Background: sunny → gradient orange, rainy → gradient blue, etc. Timestamp → time: new Date(unixTimestamp*1000).toLocaleTimeString()',
        hint_en: 'Store the API_KEY in an env variable or directly as a string (for learning only). Temp toggle: (celsius * 9/5 + 32).toFixed(1). Background: sunny → orange gradient, rainy → blue gradient, etc. Timestamp to time: new Date(unixTimestamp*1000).toLocaleTimeString()',
      },
      quiz: [
        {
          q: 'API key ko frontend JS mein hardcode karna safe hai kya?',
          options: ['Haan, completely safe', 'Nahi — source code mein visible hota hai. Public APIs ke liye theek, sensitive ones ke liye backend proxy use karo', 'Haan, browsers encrypt karte hain', 'Koi fark nahi'],
          correct: 1,
          explanation: 'Frontend JS mein sab kuch visible hai (Inspect → Sources). Weather API free mein abuse ho sakta hai. Real projects mein: API calls backend se karo, key wahan rakho. Sirf learning/demo ke liye frontend mein rakh sakte ho.',
        ...
        {
          q: 'navigator.geolocation.getCurrentPosition() kya return karta hai?',
          options: ['City name', 'Latitude aur Longitude coordinates — phir reverse geocoding se city milti hai', 'IP address', 'Country name'],
          correct: 1,
          explanation: 'Geolocation API lat/lon deta hai, city nahi. Coordinates milne ke baad weather API ko lat/lon pass karo directly, ya reverse geocoding API se city name nikalo.',
        ...
        {
          q: 'Unix timestamp ko readable date mein kaise convert karte hain?',
          options: ['timestamp.toString()', 'new Date(timestamp * 1000).toLocaleDateString()', 'Date.fromUnix(timestamp)', 'timestamp.toDate()'],
          correct: 1,
          explanation: 'Unix timestamp = seconds since 1970. JS Date milliseconds use karta hai — isliye 1000 se multiply karo. new Date(1678886400 * 1000) → Date object, phir .toLocaleDateString() se readable format.',
