...
\`\`\``,
      ...
      content_en: `## struct — Keep Related Data Together!
...
### Arrays of structs + Functions

\`\`\`c
Student class[50];
int cmpMarks(const void *a, const void *b) {
    return ((Student*)b)->marks > ((Student*)a)->marks ? 1 : -1;
}
qsort(class, count, sizeof(Student), cmpMarks);

// Pass large structs by pointer
float area(const Circle *c) { return 3.14159f * c->radius * c->radius; }
\`\`\`

### Memory Padding

\`\`\`c
// Reorder fields large-to-small to minimise padding:
typedef struct { int b; float d; char a; char c; } Efficient;
\`\`\``,
...
\`\`\``,
      ...
      content_en: `## Union — One Memory Location, Multiple Types!

### Union Basics

\`\`\`c
union Data { int i; float f; char str[20]; };
// size = largest member = 20
// Only the LAST assigned member is valid!

union Data d;
d.i = 42;      printf("%d\\n", d.i);   // 42
d.f = 3.14f;   printf("%f\\n", d.f);   // 3.14
// d.i is now garbage (float bits reinterpreted)
\`\`\`
...
typedef struct {
    Tag tag;
    union { int i; float f; char s[50]; } val;
} Variant;
...
Node* insertFront(Node *h, int d) { Node *n=createNode(d); n->next=h; return n; }
Node* insertEnd(Node *h, int d) {
    Node *n=createNode(d);
    if(!h) return n;
    Node *c=h; while(c->next) c=c->next; c->next=n; return h;
}
...
\`\`\`c
typedef struct DNode { int d; struct DNode *p,*n; } DNode;
// insert/delete O(1) with pointer — no traversal needed!
\`\`\``,
...
\`\`\``,
      ...
      content_en: `## Week 5 Project — Combine Everything!

This week covered: struct, union, enum, linked lists.

### System Architecture

\`\`\`c
typedef enum { DEPT_TECH, DEPT_HR, DEPT_SALES } Dept;
typedef enum { SKILL_C=1, SKILL_PYTHON=2, SKILL_JS=4 } Skill;

typedef struct {
    int id; char name[60];
    Dept dept; float salary;
    int skills;  // bit flags
} Employee;

typedef struct Node {
    Employee e; struct Node *next;
} Node;
\`\`\``,
