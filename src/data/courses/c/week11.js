...
\`\`\``,
      ...
      content_en: `## Graph — Network of Nodes and Edges!

### Adjacency List (Preferred)

\`\`\`c
typedef struct Node { int v,w; struct Node*n; } Node;
typedef struct { Node*head[100]; int V,E,dir; } Graph;

Graph* newGraph(int V,int dir){
    Graph*g=calloc(1,sizeof(Graph));g->V=V;g->dir=dir;return g;
}
void addEdge(Graph*g,int u,int v,int w){
    Node*n=malloc(sizeof*n);n->v=v;n->w=w;n->n=g->head[u];g->head[u]=n;
    if(!g->dir){Node*m=malloc(sizeof*m);m->v=u;m->w=w;m->n=g->head[v];g->head[v]=m;}
}
\`\`\`
...
void dfs(Graph*g,int v){
    vis[v]=1; printf("%d ",v);
    for(Node*n=g->head[v];n;n=n->n) if(!vis[n->v]) dfs(g,n->v);
}

void bfs(Graph*g,int s){
    int q[100],f=0,r=0; memset(vis,0,sizeof(vis));
    vis[s]=1;q[r++]=s;
    while(f<r){int v=q[f++];printf("%d ",v);
        for(Node*n=g->head[v];n;n=n->n) if(!vis[n->v]){vis[n->v]=1;q[r++]=n->v;}
    }
}
\`\`\``,
...
\`\`\``,
      ...
      content_en: `## Dijkstra's Shortest Path
...
void dijkstra(int g[][100], int V, int src, int *dist, int *par) {
    int vis[100]={0};
...
\`\`\`c
int stk[100],top=-1,vis[100];
void topoDFS(Graph*g,int v){
    vis[v]=1;
    for(Node*n=g->head[v];n;n=n->next) if(!vis[n->v]) topoDFS(g,n->v);
    stk[++top]=v;
}
void topoSort(Graph*g){
    memset(vis,0,sizeof(vis));top=-1;
    for(int v=0;v<g->V;v++) if(!vis[v]) topoDFS(g,v);
...
\`\`\`c
void kahn(Graph*g){
    int ind[100]={0},q[100],f=0,r=0,cnt=0;
    for(int v=0;v<g->V;v++) for(Node*n=g->head[v];n;n=n->next) ind[n->v]++;
    for(int v=0;v<g->V;v++) if(!ind[v]) q[r++]=v;
    while(f<r){
        int u=q[f++];printf("%d ",u);cnt++;
        for(Node*n=g->head[u];n;n=n->next) if(!--ind[n->v]) q[r++]=n->v;
    }
    if(cnt!=g->V) printf("\\nCYCLE!");
}
\`\`\``,
