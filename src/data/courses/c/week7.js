...
\`\`\``,
      ...
      content_en: `## C Preprocessor — The Compiler's Gatekeeper!
...
// Literally pastes file contents at this point!
\`\`\`
...
#else
    #define LOG(msg)  // zero cost in release!
#endif
...
#define INIT(x) do { x.a=0; x.b=0; } while(0)

// Stringify and token-pasting
#define STR(x)      #x          // x → "x"
#define XSTR(x)     STR(x)      // expand then stringify
#define CONCAT(a,b) a##b        // a,b → ab

// X-Macro pattern — define data once, use multiple times
#define COLORS X(RED,0xFF0000) X(GREEN,0x00FF00) X(BLUE,0x0000FF)
typedef enum { #define X(n,v) COLOR_##n, COLORS #undef X COLOR_COUNT } Color;
\`\`\`
...
### Inline Functions (Better than Macros)

\`\`\`c
static inline int square(int x) { return x*x; }  // type-safe, no side effects
static inline float squareF(float x) { return x*x; }
\`\`\``,
...
a & b  // AND  = 0b1000 = 8   (both bits 1)
a | b  // OR   = 0b1110 = 14  (either bit 1)
a ^ b  // XOR  = 0b0110 = 6   (bits differ)
~a     // NOT  = -11           (flip all bits)
a << 1 // SHL  = 20  (×2 per shift)
a >> 1 // SHR  = 5   (÷2 per shift)
\`\`\`
...
n>0 && (n&(n-1))==0;  // is power of 2?
n & (-n);              // lowest set bit
__builtin_popcount(n); // count 1-bits (GCC)
\`\`\`
...
\`\`\``,
      ...
      content_en: `## Week 7 Project — Combine Everything!

### Config System with Macros

\`\`\`c
#define CONFIG_MAX_SECTIONS 16
#define CONFIG_MAX_KEYS     64
#define VALIDATE_PORT(p)    ((p)>=1 && (p)<=65535)

#define CONFIG_GET_INT(cfg,s,k,d)  config_get_int(cfg,s,k,d)
#define CONFIG_GET_STR(cfg,s,k,d)  config_get_str(cfg,s,k,d)
\`\`\`

### Bitfields — 8 booleans in 1 byte!
