...
\`\`\``,
      ...
      content_en: `## O(n²) Sorting Algorithms
...
void bubble(int a[],int n){
    for(int i=0;i<n-1;i++){
        int sw=0;
        for(int j=0;j<n-i-1;j++)
            if(a[j]>a[j+1]){int t=a[j];a[j]=a[j+1];a[j+1]=t;sw=1;}
        if(!sw)break;  // early exit if already sorted
    }
}
// O(n²) worst, O(n) best (sorted). Stable. In-place.
\`\`\`
...
// O(n²) always. NOT stable. Min swaps (n-1).
\`\`\`
...
// O(n²) worst, O(n) best. Stable. Best for small/nearly-sorted.
\`\`\``,
...
\`\`\``,
      ...
      content_en: `## O(n log n) Sorts — Professional Speed!
...
void qsort_(int a[],int lo,int hi){
    ...
    qsort_(a,lo,p-1); qsort_(a,p+1,hi);
}
// O(n log n) avg, O(n²) worst (sorted input). NOT stable. In-place.
\`\`\``,
...
\`\`\``,
      ...
      content_en: `## Advanced Sorts — Fastest for Special Cases!
...
// O(n+k), Stable, FASTEST when k=O(n)
\`\`\`
...
// O(d*(n+10)) = O(n) for fixed-width integers
\`\`\``,
...
\`\`\``,
      ...
      content_en: `## Week 10 Project — Become a Sorting Master!

### stdlib qsort() and bsearch()
...
int key=42;
int *p=(int*)bsearch(&key,arr,n,sizeof(int),cmpAsc);
if(p) printf("Found %d\\n",*p);
\`\`\`
...
Guaranteed O(nlogn)→ Heap Sort
Small int range    → Counting Sort
Fixed-width ints   → Radix Sort
General purpose    → qsort() (IntroSort)
\`\`\``,
