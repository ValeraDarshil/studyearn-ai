...
export function useCodeLearn(language) {
  ...
  const isSectionUnlocked = useCallback((weekNumber, sectionIndex, sections, allWeeks = []) => {
    if (isAdmin) return true; // Admin bypass — all sections unlocked
    if (weekNumber === 1 && sectionIndex === 0) return true; // First section always unlocked

    // Same week — previous section's quiz must be PASSED (>=70%)
    if (sectionIndex > 0) {
      const prevSection = sections[sectionIndex - 1];
      return isSectionQuizPassed(prevSection.id);
    }

    // First section of a new week — EVERY section of prev week must be quiz passed
    // "some" was wrong — it was unlocking next week even if only 1 section was passed
    if (weekNumber > 1 && allWeeks.length > 0) {
      const prevWeek = allWeeks.find(w => w.week === weekNumber - 1);
      if (!prevWeek) return false;
      return prevWeek.sections.every(s => isSectionQuizPassed(s.id));
    }
