...
// Preload audio immediately on module load — eliminates first-play decode delay.
// The browser decodes the base64 and buffers the audio in the background.
// By the time user watches an ad and +1 triggers, Audio is instant.
let glitchAudioCache: HTMLAudioElement | null = null;
try {
  glitchAudioCache = new Audio(GLITCH_SFX_B64);
  glitchAudioCache.volume = 0.6;
  glitchAudioCache.preload = 'auto';
  // Trigger silent decode — browsers decode on first interaction
  glitchAudioCache.load();
...
function playGlitchSFX() {
  try {
    ...
    glitchAudioCache.play().catch(() => { /* autoplay blocked — skip */ });
  } catch { /* silently skip */ }
}
...
function GlitchNumber({ value, className = '' }: { value: number; className?: string }) {
  ...
  useEffect(() => {
    ...
    const wasIncrease = value > prevRef.current;
    ...
    // SFX only on increase (e.g. 14→15 after Watch Video or refill)
    // NOT on decrease (e.g. 15→14 after asking a question)
    if (wasIncrease) playGlitchSFX();
