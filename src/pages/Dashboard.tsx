        ...
        <div className="glass rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {ACHIEVEMENTS.map((ach) => {
              ...
              // compute progress for locked ones
              ...
              return (
                <div
                  key={ach.id}
                  className={`relative p-3.5 rounded-xl border transition-all duration-300 overflow-hidden group
                    ${unlocked
                      ? `bg-gradient-to-br ${styles.bg} ${styles.border} hover:scale-[1.02] ${styles.glow ? `hover:shadow-lg hover:${styles.glow}` : ''}`
                ...
                >
                  ...
                  <div className="flex items-start gap-2.5">
                    {/* Icon */}
                    <div className={`text-xl flex-shrink-0 transition-all duration-300 ${unlocked ? 'scale-110' : 'grayscale opacity-40'}`}>
                      {unlocked ? ach.icon : <Lock className="w-4 h-4 text-slate-600 mt-0.5" />}
                    </div>

                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                        <h3 className={`text-xs font-bold leading-tight ${unlocked ? 'text-white' : 'text-slate-500'}`}>
                          {ach.name}
                        </h3>
                        <span className={`text-[8px] font-semibold px-1.5 py-0.5 rounded-full uppercase tracking-wide ${styles.badge}`}>
                          {ach.rarity}
                        </span>
                      </div>
                      <p className={`text-[10px] leading-snug ${unlocked ? 'text-slate-400' : 'text-slate-600'}`}>
                        {ach.desc}
                      </p>
                      ...
                      {!unlocked && (
                        <div className="mt-1.5">
                          ...
                          <p className="text-[9px] text-slate-600 mt-0.5">{currentVal}/{ach.threshold}</p>
                        </div>
                      )}
                      ...
                      {unlocked && (
                        <div className="flex items-center gap-1 mt-1">
                          <span className="text-[9px] font-semibold text-green-400">✓ Unlocked</span>
                          {ach.reward > 0 && (
                            <span className="text-[9px] text-yellow-500">· +{ach.reward} pts</span>
                          )}
                        </div>
                      )}
                    </div>
