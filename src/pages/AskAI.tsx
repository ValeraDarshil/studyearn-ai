...
import { AskAISkeleton } from "../components/Skeleton";
...
export function AskAI() {
  ...
  const [localQuestionsLeft, setLocalQuestionsLeft] = React.useState<number | null>(null);
  const effectiveQuestionsLeft = localQuestionsLeft ?? questionsLeft;
  ...
  const canAsk = (!!question.trim() || !!uploadedFile) && effectiveQuestionsLeft > 0 && !loading;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* ── Loading Skeleton ─────────────────────────────────────────── */}
      {loading && <AskAISkeleton step={loadingStep || "AI is thinking…"} />}
      ...
      {showAnswer && !loading && (
        <div className="rounded-2xl overflow-hidden border border-white/10 bg-white/[0.02] animate-slide-up">
          ...
          {/* Answer body — full markdown + LaTeX rendering */}
