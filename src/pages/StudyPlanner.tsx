...
// ─────────────────────────────────────────────────────────────
interface StudyDay {
  date: string;       // "Mon, 10 Mar"
  day: string;        // "Monday"
  tasks: string[];    // ["Maths: Ch 5 - Integration (2hr)", ...]
  completed: boolean[];
}
...
// Parse AI timetable text → StudyDay[]
function parseTimetable(text: string, examDate: string): StudyDay[] {
  ...
  for (let i = 0; i < Math.min(blocks.length, dayDiff); i++) {
    ...
    const tasks = block.split("\n")
      .map(l => l.replace(/^[-•*]\s*/, "").trim())
      .filter(l => l.length > 10 && !l.match(/^(Day|DAY)\s*\d+/i) && !l.startsWith("**Day"))
      .slice(0, 5);

    if (tasks.length > 0) {
      days.push({ date: dateLabel, day: dayName, tasks, completed: new Array(tasks.length).fill(false) });
    }
  }
  return days;
}
...
export function StudyPlanner() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("studyearn_plan");
    if (saved) {
      try { setPlan(JSON.parse(saved)); } catch {}
    }
  }, []);

  const savePlan = (p: SavedPlan) => {
    localStorage.setItem("studyearn_plan", JSON.stringify(p));
    setPlan(p);
  };
...
- Include chapter names or topics (make them realistic for Indian students)
...
Continue for all ${daysLeft} days.`;
    ...
    try {
      ...
      if (days.length < 1) throw new Error("Could not generate plan. Please try again.");
  ...
  const toggleTask = (dayIdx: number, taskIdx: number) => {
    if (!plan) return;
    const updated = { ...plan };
    updated.days = plan.days.map((d, di) => {
      if (di !== dayIdx) return d;
      const comp = [...d.completed];
      comp[taskIdx] = !comp[taskIdx];
  ...
  const clearPlan = () => {
    localStorage.removeItem("studyearn_plan");
    setPlan(null); setExamName(""); setExamDate("");
    setSubjects([""]); setError("");
  };

  // Today's index in the plan
  const todayIdx = plan ? (() => {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Exam date + subjects → AI creates your perfect daily timetable
        </p>
      </div>
      ...
      <div className="rounded-2xl p-6 border border-white/10 bg-white/[0.02] space-y-5">
        ...
        <div className="space-y-2">
          ...
          {examDate && (
            <p className="text-xs text-purple-300">
              📅 {getDaysUntil(examDate)} days left to prepare
            </p>
          )}
        </div>
          ...
          {subjects.map((s, i) => (
            <div key={i} className="flex gap-2">
              <input value={s} onChange={e => {
                  const upd = [...subjects]; upd[i] = e.target.value; setSubjects(upd);
                }}
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          ...
          <span>AI generates personalized plan • Earn <strong className="text-purple-300">+20 pts</strong></span>
        </div>
            ...
            {plan.days[activeDay].tasks.map((task, ti) => (
              <button key={ti} onClick={() => toggleTask(activeDay, ti)}
                className={`w-full flex items-start gap-3 p-3 rounded-xl border text-left transition-all
                  ${plan.days[activeDay].completed[ti]
                ...
                <span className={`text-sm leading-relaxed transition-colors
                  ${plan.days[activeDay].completed[ti] ? "text-slate-500 line-through" : "text-slate-300"}`}>
                  {task}
                </span>
              </button>
            ))}
