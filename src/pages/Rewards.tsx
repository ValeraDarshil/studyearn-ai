...
export function Rewards() {
  ...
  const { points, streak, userId, unlockedAchievements, userStats } = useApp();
  ...
  const loadLeaderboard = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/leaderboard/top?limit=3`,
      );
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-slate-500 font-medium px-2 py-1 glass rounded-lg border border-white/5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {ACHIEVEMENTS.map((ach) => {
            ...
            return (
              <div key={ach.id} className={`relative glass rounded-xl p-4 border transition-all duration-300 overflow-hidden group card-shine
                ${unlocked ? `bg-gradient-to-br ${styles.bg} ${styles.border} hover:scale-[1.02] ${styles.glow ? `hover:shadow-lg` : ''}` : 'border-white/5 hover:border-white/10'}`}>
                <div className="flex items-start gap-3">
                  <div className={`text-2xl flex-shrink-0 transition-all ${unlocked ? 'scale-110' : 'grayscale opacity-40'}`}>
                    {ach.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {!unlocked && (
                      <div className="mt-1.5">
                        <div className="w-full h-1 rounded-full bg-white/5 overflow-hidden">
                          <div className="h-full rounded-full bg-gradient-to-r from-slate-600 to-slate-500" style={{ width: `${prog}%` }} />
                        </div>
                        <span className="text-[9px] text-slate-600">{val}/{ach.threshold}</span>
                      </div>
                    )}
                    {unlocked && (
                      <span className="text-[9px] font-semibold text-green-400">✓ Unlocked{ach.reward > 0 ? ` · +${ach.reward}pts` : ''}</span>
                    )}
                  </div>
                </div>
              </div>
