...
const api = {
  imgToPDF: async (files: File[]) => {
    const fd = new FormData();
    files.forEach((f) => fd.append("files", f));
    return callPDF("img-to-pdf", fd);
  },
  mergePDF: async (files: File[]) => {
    const fd = new FormData();
    files.forEach((f) => fd.append("files", f));
    return callPDF("merge-pdf", fd);
  },
  splitPDF: async (file: File, pages: string) => {
    const fd = new FormData();
    fd.append("file", file);
    fd.append("pages", pages);
    return callPDF("split-pdf", fd);
  },
  compressPDF: async (file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    return callPDF("compress-pdf", fd);
  },
  rotatePDF: async (file: File, deg: number) => {
    const fd = new FormData();
    fd.append("file", file);
    fd.append("degrees", String(deg));
    return callPDF("rotate-pdf", fd);
  },
  pageNumbers: async (file: File, pos: string) => {
    const fd = new FormData();
    fd.append("file", file);
    fd.append("position", pos);
    return callPDF("pdf-page-numbers", fd);
  },
  watermark: async (file: File, text: string) => {
    const fd = new FormData();
    fd.append("file", file);
    fd.append("text", text);
    return callPDF("pdf-watermark", fd);
  },
};
...
const TOOLS: Tool[] = [
  {
    id: "img-pdf",
    title: "Images → PDF",
    seoTitle: "Image to PDF Converter — Free Online",
  ...
  },
  {
    id: "merge-pdf",
    title: "Merge PDFs",
    seoTitle: "Merge PDF Files Free Online",
  ...
  },
  {
    id: "rotate-pdf",
    title: "Rotate PDF",
    seoTitle: "Rotate PDF Pages Free Online",
  ...
  },
  {
    id: "watermark",
    title: "Watermark PDF",
    seoTitle: "Add Watermark to PDF Free Online",
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold text-white truncate">
              🚀 StudyEarn AI — Free AI Platform for Students
            </p>
            <p className="text-[11px] text-slate-400 truncate">
              Ask AI • PPT Generator • Quiz • Daily Rewards — 100% Free
            </p>
          </div>
        </div>
      ...
      >
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="text-center space-y-2">
            <h3 className="text-xl font-extrabold text-white">
              ...
              <span style={{ color: "#a78bfa" }}>
                Aur bhi powerful tools hain!
              </span>
            </h3>
            <p className="text-sm text-slate-400">
              Free account banao — AI study assistant, PPT generator, quiz maker
              + real rewards!
            </p>
          </div>
          ...
          <a
            ...
            style={{ background: "linear-gradient(135deg,#7c3aed,#4f46e5)" }}
          >
            <Sparkles className="w-4 h-4" /> Create Free Account — Get 50 Bonus
            Points 🎁
          </a>
          <button
            onClick={onClose}
...
function BottomPromo() {
  return (
    <div
      ...
      style={{
        background: "linear-gradient(135deg,#1e1b4b,#0f172a,#1a1040)",
        border: "1px solid rgba(124,58,237,0.3)",
      }}
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-extrabold text-white text-sm">
              PDF tools sirf shuruat hai! ✨
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
              StudyEarn AI pe free signup karo — AI assistant, PPT generator,
              quiz, daily rewards aur bahut kuch!
            </p>
          </div>
        </div>
...
export function PDFToolsPublic() {
  ...
  const [result, setResult] = useState<{ url: string; info?: string } | null>(
    null,
  );
  ...
  const [pnPos, setPnPos] = useState<"bottom-center" | "bottom-right">(
    "bottom-center",
  );
  const addFiles = (fl: FileList | null) => {
    ...
    setFiles((prev) =>
      Array.from(
        new Map([...prev, ...arr].map((f) => [f.name + f.size, f])).values(),
      ),
    );
  };
  ...
  const convert = async () => {
    ...
    if (tool.id === "merge-pdf" && files.length < 2) {
      setError("Kam se kam 2 PDF files chahiye merge karne ke liye.");
      return;
    }
    setBusy(true);
    setError("");
    ...
    try {
      ...
      switch (tool.id) {
        case "img-pdf":
          res = await api.imgToPDF(files);
          break;
        case "merge-pdf":
          res = await api.mergePDF(files);
          break;
        case "split-pdf":
          res = await api.splitPDF(files[0], splitPages);
          break;
        case "compress-pdf":
          res = await api.compressPDF(files[0]);
          break;
        case "rotate-pdf":
          res = await api.rotatePDF(files[0], rotateDeg);
          break;
        case "page-numbers":
          res = await api.pageNumbers(files[0], pnPos);
          break;
        case "watermark":
          res = await api.watermark(files[0], wmText);
          break;
        default:
  ...
  return (
    <>
      <Helmet>
        <title>
          {tool
            ? `${tool.seoTitle} | StudyEarn AI`
            : "Free PDF Tools Online — Image to PDF, Merge, Compress & More | StudyEarn AI"}
        </title>
        <meta
          name="description"
          content={
            tool
              ? `${tool.seoTitle} — free, no login required, no watermark. Works on mobile and desktop. By StudyEarn AI.`
              : "Free online PDF tools — Image to PDF, Merge PDF, Compress, Split, Rotate, Add Page Numbers, Watermark. No login required. No watermark added. 100% free by StudyEarn AI."
          }
        />
        <meta
          property="og:title"
          content="Free PDF Tools Online — StudyEarn AI"
        />
        <meta
          property="og:description"
          content="Free online PDF tools — Image to PDF, Merge, Compress, Split & more. No login required. By StudyEarn AI."
        />
        <link rel="canonical" href="https://studyearnai.tech/pdf-tools" />
      </Helmet>
      ...
      <div className="min-h-screen" style={{ background: "#0a0a0f" }}>
        ...
        <main className="max-w-4xl mx-auto px-4 py-6">
          ...
          {!isLoggedIn && banner && (
            <TopBanner onClose={() => setBanner(false)} />
          )}
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
                  <FileText className="w-6 h-6 text-cyan-400" /> Free PDF Tools
                  Online
                </h1>
                <p className="text-sm text-slate-400">
                  Image to PDF, Merge, Compress, Split, Watermark & more — free,
                  no login required, no watermark added
                </p>
                ...
                {TOOLS.filter((t) => t.cat === cat).map((t) => (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg,rgba(124,58,237,0.07),rgba(79,70,229,0.04))",
                      }}
                    />
                    <div className="relative flex items-start gap-4">
                      <div
                        className={`w-11 h-11 rounded-xl bg-gradient-to-br ${t.grad} flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform`}
                      >
              ...
              >
                <h2 className="text-sm font-bold text-white">
                  About StudyEarn AI Free PDF Tools
                </h2>
                <p className="text-xs text-slate-500 leading-relaxed">
                  StudyEarn AI provides free online PDF tools — no download, no
                  installation, no signup required. Convert images to PDF, merge
                  multiple PDFs, compress large files, split documents, rotate
                  pages, add page numbers, watermarks and more. All tools work
                  on mobile and desktop. Your files are processed securely and
                  not stored on our servers.
                </p>
                <p className="text-xs text-slate-500 leading-relaxed">
                  StudyEarn AI is also a free AI-powered study platform for
                  Indian students — unlimited AI questions, PPT generator, quiz
                  creator, study planner, and real rewards. Perfect for JEE,
                  NEET, UPSC, and board exams.
                </p>
              </div>
            </div>
          )}
          ...
          {tool && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-5 h-5 rounded-md bg-gradient-to-br ${tool.grad} flex items-center justify-center`}
                  >
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Free • No login • No watermark added • Works on all devices
                </p>
              </div>
              ...
              {tool.extra === "split" && !result && (
                ...
                >
                  <label className="text-xs font-semibold text-slate-400">
                    Konse pages extract karne hain?
                  </label>
                  <input
                    value={splitPages}
                  ...
                  <p className="text-xs text-slate-600">
                    "1-5" = pages 1 to 5 &nbsp;·&nbsp; "1,3,7" = specific
                    &nbsp;·&nbsp; "all" = sab
                  </p>
                </div>
              )}
              ...
              {tool.extra === "watermark" && !result && (
                ...
                >
                  ...
                  <input
                    value={wmText}
                    onChange={(e) => setWmText(e.target.value.slice(0, 30))}
                  ...
                  />
                  <p className="text-xs text-slate-600">
                    Max 30 characters • Diagonal pe stamp hoga
                  </p>
                </div>
              )}
                ...
                >
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    {tool.accept
                      .split(",")
                      .map((a) =>
                        a.split("/").pop()?.replace(".", "")?.toUpperCase(),
                      )
                      .filter(Boolean)
                      .join(" / ")}
                  </p>
              ...
              {error && (
                <div
                  className="text-sm text-red-400 rounded-xl px-4 py-3"
                  style={{
              ...
              {busy && (
                ...
                >
                  ...
                  <div
                    className="h-1.5 rounded-full max-w-xs mx-auto overflow-hidden"
              ...
              {result && (
                ...
                >
                  ...
                  <button
                    onClick={() => download(result.url, fname || tool.filename)}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-white">
                          Signup karo aur +10 points earn karo is conversion ke
                          liye! 🎁
                        </span>
                      </div>
                      <p className="text-[11px] text-slate-400">
                        Logged-in users ko har PDF conversion pe points milte
                        hain. Plus AI tools, PPT generator & daily rewards!
                      </p>
                      <a
                        href="/signup"
                  ...
                  <button
                    onClick={reset}
                    className="w-full text-xs text-slate-600 hover:text-white transition-colors py-1"
                  >
        ...
        >
          <div className="max-w-4xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-slate-600">
              © 2026 StudyEarn AI — Free PDF Tools & AI Study Platform for
              Students
            </p>
