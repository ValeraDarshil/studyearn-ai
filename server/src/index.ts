 ...
 *   4. DB connect karna
 *   5. Server start karna
 *   6. Background jobs (premium polling, mastery decay)
 *
 * Business logic KABHI yahan nahi hogi.
 * Saari logic in folders mein hai:
 *   controllers/ → request/response handle karna
...
// Diagnostic endpoint — checks AI provider keys WITHOUT ever logging
// or returning the key values. Only presence + length is exposed, so
// you can tell "missing" vs "trailing whitespace" vs "wrong value"
// apart without leaking secrets in the response.
//
// NOTE: this is intentionally unauthenticated for quick debugging.
// Remove or gate behind an admin check once the current AI outage
// is resolved — it's a (small) information-disclosure surface.
app.get('/health/ai', (_req, res) => {
  const describe = (value: string | undefined) => ({
    present: !!value,
    length: value?.length ?? 0,
    trimmedLength: value?.trim().length ?? 0, // mismatch → trailing space/newline in the value
  });

  res.json({
    groq: describe(process.env.GROQ_API_KEY),
    openrouter: describe(process.env.OPENROUTER_API_KEY),
    nvidia: describe(process.env.NVIDIA_API_KEY),
...
connectDB().then(() => {
  ...
  // ── Nightly mastery decay — spaced repetition forgetting curve ──
  //
  // Wrapped in !cluster.isWorker guard. cluster.ts forks up to 4 worker
  // processes. Without this guard, every worker would run the cron
  // independently — each user decayed 4× per night instead of 1×,
  // making mastery scores drop 4× faster than intended and incorrectly
  // flagging topics as isWeak. Only the primary process (or a
  // single-instance deploy) runs the decay cron.
  if (!cluster.isWorker) {
    setInterval(async () => {
      try {
        // Only decay users active in last 30 days (not ALL users).
