...
async function handlePDFAction(req: Request, actionLabel: string): Promise<void> {
  ...
  try {
    ...
    logger.error({ err: err.message }, 'handlePDFAction error');
  }
}
...
export async function imgToPDF(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`IMG→PDF: ${files.length} files in ${Date.now() - start}ms`);
    handlePDFAction(req, 'Converted images to PDF').catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'IMG→PDF ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
...
export async function mergePDF(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`MERGE-PDF: ${files.length} files in ${Date.now() - start}ms`);
    handlePDFAction(req, 'Merged PDF files').catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'MERGE PDF ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
...
export async function splitPDFHandler(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`SPLIT-PDF in ${Date.now() - start}ms`);
    handlePDFAction(req, 'Split PDF pages').catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'SPLIT PDF ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
...
export async function compressPDFHandler(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`COMPRESS-PDF: ${originalKB}KB → ${compressedKB}KB (${savingsPercent}% saved) in ${Date.now() - start}ms`);
    handlePDFAction(req, 'Compressed PDF').catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'COMPRESS PDF ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
...
export async function rotatePDFHandler(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`ROTATE-PDF: ${deg}° in ${Date.now() - start}ms`);
    handlePDFAction(req, 'Rotated PDF').catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'ROTATE PDF ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
...
export async function pageNumbersHandler(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`PAGE-NUMBERS in ${Date.now() - start}ms`);
    handlePDFAction(req, 'Added page numbers to PDF').catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'PAGE NUMBERS ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
...
export async function watermarkHandler(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`WATERMARK: "${text}" in ${Date.now() - start}ms`);
    handlePDFAction(req, 'Added watermark to PDF').catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'WATERMARK ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
...
export async function officeToPDFHandler(req: Request, res: Response) {
  ...
  try {
    ...
    logger.info(`OFFICE→PDF: ${req.file.originalname} in ${Date.now() - start}ms`);
    handlePDFAction(req, `Converted ${req.file.originalname} to PDF`).catch((e: unknown) => logger.error({ err: e }, 'background task failed'));
  } catch (err: any) {
    logger.error({ err: err.message }, 'OFFICE→PDF ERROR');
    const isLOError = err.message.includes('LibreOffice') || err.message.includes('soffice');
    res.status(500).json({
      success: false,
      message: isLOError
