...
async function checkPremiumEligibility(userId: string): Promise<{ ok: boolean; reason?: string }> {
  try {
    ...
    logger.error({ err: err.message }, 'Fraud check error');
    return { ok: false, reason: 'Verification failed. Please try again.' };
  }
}
  ...
  try {
    ...
    for (const red of pending) {
      try {
        ...
        logger.error({ err: err.message, email: (red as any).userEmail }, 'Premium processing error');
      }
    }
  } catch (err: any) {
    logger.error({ err: err.message }, 'Premium polling error');
  }
}
...
export async function fixStuckRedemptions(): Promise<void> {
  try {
    ...
    logger.error({ err: err.message }, 'fixStuckRedemptions error');
  }
}
...
export async function redeemReward(req: Request, res: Response) {
  try {
    ...
    logger.error({ err: err.message }, 'REDEEM ERROR');
    res.status(500).json({ success: false, message: err.message });
  }
}
